class Review:
    ...
    def get(dbconnect, id):
        ...
        cursor.execute("SELECT id,user_for,user_from,amount_of_stars,title,review_text FROM review WHERE id = %s",
                       (id,))
        id, user_for, user_from, amount_of_stars, title, review_text = cursor.fetchone()
        return Review(id, user_for, user_from, amount_of_stars, title, review_text)
