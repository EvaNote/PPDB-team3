class User:
    ...
    def get(cursor, id):
        cursor.execute(
            "SELECT id, email, first_name, last_name, age, gender, active_since, picture, address FROM user WHERE id = %s",
            (id,))
    ...
    def to_dict(self):
        return {'id': self.id, ' email': self.email, 'first_name': self.first_name, 'last_name': self.last_name,
                'age': self.age, 'gender': self.gender, 'active_since': self.active_since, 'picture': self.picture,
